...
class Client:
    ...
    def user_recent(self,
        ...
        game_mode : GameMode, optional
            The game mode to look up stats for. Defaults to osu! standard.
        limit : int, optional
            The number of results to return in the range [1, 50].

        Returns
        -------
        high_scores : list[HighScore]
            The user's recent scores.
        """
        user, type_ = self._user_and_type(user_name, user_id, required=True)

        if not (1 <= limit <= 50):
            raise ValueError(
                f'limit must be in the range [1, 50], got: {limit!r}',
            )

        response = requests.get(
            self.api_url + '/get_user_recent',
            params={
                'k': self.api_key,
                'u': user,
                't': type_,
        ...
        as_dicts = (
            {self._user_recent_aliases.get(k, k): v for k, v in beatmap.items()}
            for beatmap in response.json()
        )
        return [
            HighScore(
                client=self,
                **{
                    k: self._user_recent_conversions[k](v)
                    for k, v in d.items()
                    if k in self._user_recent_conversions
                },
                _user=_user_ob,
                pp=-1.0
            )
    ...
    def beatmap_best(self,
        ...
        response = requests.get(
            ...
            params={
                ...
                't': type_,
                'm': int(game_mode),
                'mods': int(reduce(lambda x, y: int(x) | int(y), mods)) if mods != set() else None,
                'limit': limit,
            },
        )
        response.raise_for_status()
